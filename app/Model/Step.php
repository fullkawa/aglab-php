 ...
 */
	...
	public function beforeSave($options = array()) {
		...
		if (@$this->data['Step']['context']) {
			$this->log("[Step::beforeSave()] context->" . json_encode($this->data['Step']['context']), LOG_DEBUG);
			$this->data['Step']['context'] = serialize($this->data['Step']['context']);
			///$this->log("[Step::beforeSave()] context->" . json_encode($this->data['Step']['context']), LOG_DEBUG);
		}
		return true;
	}
	...
	public function afterFind($results, $primary = false) {
		...
		foreach ($results as &$result) {
			if (@$result['Step']['context']) {
				$result['Step']['context'] = unserialize($result['Step']['context']);
			}
		}
		return $results;
	}
	...
	public function next($data = array()) {
		...
		///$this->log('[Step::next()] data -> ' . json_encode($this->data), LOG_DEBUG);
		...
		$next_status = Step::STATUS_NOT_EXECUTED;
		if ($next_context['stage'] == 'ending') {
			$next_status = Step::STATUS_EXECUTED;
		}
		$next_data = array(
		...
		///$this->log('[Step::next()] next_data -> ' . json_encode($next_data), LOG_DEBUG);
	...
	public function _markAsExecuted($id) {
		$this->id = $id;
		$result = $this->saveField('status', Step::STATUS_EXECUTED);
		return $result;
	}
}
